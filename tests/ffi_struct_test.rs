...
// ============================================================================
// Prefix/Suffix Attribute Tests
// ============================================================================

#[test]
fn test_set_prefix_attribute() {
    unsafe {
        let handle = praeda_generator_new();
        assert!(!handle.is_null());
        ...
        let result = praeda_generator_set_prefix_attribute(
            handle,
            CString::new("weapon").unwrap().as_ptr(),
            CString::new("sword").unwrap().as_ptr(),
            CString::new("sharp").unwrap().as_ptr(),
            CString::new("damage").unwrap().as_ptr(),
            5.0,
            0.0,
            15.0,
        ...
        assert_eq!(result, 0, "Setting prefix attribute should succeed");
...
fn test_set_suffix_attribute() {
    unsafe {
        ...
        let result = praeda_generator_set_suffix_attribute(
            handle,
            CString::new("weapon").unwrap().as_ptr(),
            CString::new("sword").unwrap().as_ptr(),
            CString::new("of awesomeness").unwrap().as_ptr(),
            CString::new("attack_speed").unwrap().as_ptr(),
            0.5,
            0.0,
            1.5,
            0,
        );
        assert_eq!(result, 0, "Setting suffix attribute should succeed");
...
#[test]
fn test_set_prefix_attribute_null_handle() {
    unsafe {
        let invalid_handle: *mut PraedaGeneratorHandle = std::ptr::null_mut();
        ...
        assert_eq!(result, -1, "Setting prefix on null handle should fail");
    }
}

#[test]
fn test_set_suffix_attribute_null_handle() {
    unsafe {
        let invalid_handle: *mut PraedaGeneratorHandle = std::ptr::null_mut();
        ...
        assert_eq!(result, -1, "Setting suffix on null handle should fail");
    }
}

#[test]
fn test_generate_loot_with_prefix_suffix() {
    unsafe {
        let handle = praeda_generator_new();
        assert!(!handle.is_null());

        // Configure with prefixes and suffixes
        let _ = praeda_generator_set_quality_data(handle, CString::new("common").unwrap().as_ptr(), 100);
        let _ = praeda_generator_set_item_type(handle, CString::new("weapon").unwrap().as_ptr(), 1);
        let _ = praeda_generator_set_item_subtype(
            handle,
        ...
        // Set prefix attribute
        let _ = praeda_generator_set_prefix_attribute(
            handle,
            CString::new("weapon").unwrap().as_ptr(),
            CString::new("sword").unwrap().as_ptr(),
            CString::new("sharp").unwrap().as_ptr(),
        ...
        // Set suffix attribute
        let _ = praeda_generator_set_suffix_attribute(
            handle,
            CString::new("weapon").unwrap().as_ptr(),
            CString::new("sword").unwrap().as_ptr(),
            CString::new("of awesomeness").unwrap().as_ptr(),
        ...
        // Generate with high affix chance to get affixes
        let mut error_ptr = std::ptr::null_mut();
        let array_handle = praeda_generator_generate_loot(
            handle,
            5,     // number_of_items
        ...
        assert!(!array_handle.is_null(), "Item array should not be null");
        assert_eq!(praeda_item_array_count(array_handle), 5);
...
#[test]
fn test_set_item_names_null_handle() {
    unsafe {
        let invalid_handle: *mut PraedaGeneratorHandle = std::ptr::null_mut();
        let names = vec![CString::new("sword").unwrap()];
        let name_ptrs: Vec<*const c_char> = names.iter().map(|s| s.as_ptr()).collect();
        ...
        assert_eq!(result, -1, "Setting item names on null handle should fail");
    }
}
